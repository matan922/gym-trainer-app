...
api.interceptors.response.use(
	...
	async error => {
		...
		if ((status === 401 || status === 403) && !originalRequest._retry && error.config.url !== "/auth/token") {
			...
			try {
				...
				if (!refreshTokenPromise) {
					refreshTokenPromise = api.post('/auth/token').finally(() => { refreshTokenPromise = null })
				}
...
export const fetchClient = async (id: string) => {
	const response = await api.get<{ client: Client, workouts: Workout[] }>(`/clients/${id}`);
	return response;
};
...
export const putClient = async (clientData: Client, id: string) => {
	const response = await api.put<{ client: Client, success: boolean }>(`/clients/${id}`, clientData)
	return response
}
...
export const getWorkout = async (clientId: string, workoutId: string) => {
	const response = await api.get<Workout>(`/clients/${clientId}/workouts/${workoutId}`)
	return response.data
}

export const postWorkout = async (clientId: string, workoutData: Workout) => {
	const response = await api.post<Workout>(`/clients/${clientId}/workouts`, workoutData)
	return response
}

export const putWorkout = async (clientId: string, workoutData: Workout, workoutId: string) => {
	const response = await api.put<Workout>(`/clients/${clientId}/workouts/${workoutId}`, workoutData)
	return response.data
}

export const deleteWorkout = async (clientId: string, workoutId: string) => {
	const response = await api.delete<Workout>(`/clients/${clientId}/workouts/${workoutId}`)
	return response
}
